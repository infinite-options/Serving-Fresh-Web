    ...
    if form.validate_on_submit():
        ...
        add_kitchen = db.put_item(TableName='kitchens',
                                  Item={'kitchen_id': {'S': kitchen_id},
                                        ...
                                        'delivery_close_time': {
                                            'S': form.deliveryCloseTimeMonday.data.strftime('%H:%M')},
...
def registerCustomer():
    ...
    return render_template('registerCustomer.html', title='registerCustomer',
                           form=form)  # This is what happens if the submit is unsuccessful with errors highlighted
...
def kitchenSettings(id):
    ...
    if form.validate_on_submit():
        ...
        form.acceptingOpenTimeMonday.data = datetime.strptime(kitchen['accepting_hours']['L'][1]['M']['open_time']['S'],
                                                              '%H:%M')
        form.acceptingOpenTimeTuesday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][2]['M']['open_time']['S'], '%H:%M')
        form.acceptingOpenTimeWednesday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][3]['M']['open_time']['S'], '%H:%M')
        form.acceptingOpenTimeThursday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][4]['M']['open_time']['S'], '%H:%M')
        form.acceptingOpenTimeFriday.data = datetime.strptime(kitchen['accepting_hours']['L'][5]['M']['open_time']['S'],
                                                              '%H:%M')
        form.acceptingOpenTimeSaturday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][6]['M']['open_time']['S'], '%H:%M')
        form.acceptingOpenTimeSunday.data = datetime.strptime(kitchen['accepting_hours']['L'][0]['M']['open_time']['S'],
                                                              '%H:%M')
        form.acceptingCloseTimeMonday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][1]['M']['close_time']['S'], '%H:%M')
        form.acceptingCloseTimeTuesday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][2]['M']['close_time']['S'], '%H:%M')
        form.acceptingCloseTimeWednesday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][3]['M']['close_time']['S'], '%H:%M')
        form.acceptingCloseTimeThursday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][4]['M']['close_time']['S'], '%H:%M')
        form.acceptingCloseTimeFriday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][5]['M']['close_time']['S'], '%H:%M')
        form.acceptingCloseTimeSaturday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][6]['M']['close_time']['S'], '%H:%M')
        form.acceptingCloseTimeSunday.data = datetime.strptime(
            kitchen['accepting_hours']['L'][0]['M']['close_time']['S'], '%H:%M')
        ...
        form.deliveryOpenTimeMonday.data = datetime.strptime(kitchen['delivery_hours']['L'][1]['M']['open_time']['S'],
                                                             '%H:%M')
        form.deliveryOpenTimeTuesday.data = datetime.strptime(kitchen['delivery_hours']['L'][2]['M']['open_time']['S'],
                                                              '%H:%M')
        form.deliveryOpenTimeWednesday.data = datetime.strptime(
            kitchen['delivery_hours']['L'][3]['M']['open_time']['S'], '%H:%M')
        form.deliveryOpenTimeThursday.data = datetime.strptime(kitchen['delivery_hours']['L'][4]['M']['open_time']['S'],
                                                               '%H:%M')
        form.deliveryOpenTimeFriday.data = datetime.strptime(kitchen['delivery_hours']['L'][5]['M']['open_time']['S'],
                                                             '%H:%M')
        form.deliveryOpenTimeSaturday.data = datetime.strptime(kitchen['delivery_hours']['L'][6]['M']['open_time']['S'],
                                                               '%H:%M')
        form.deliveryOpenTimeSunday.data = datetime.strptime(kitchen['delivery_hours']['L'][0]['M']['open_time']['S'],
                                                             '%H:%M')
        form.deliveryCloseTimeMonday.data = datetime.strptime(kitchen['delivery_hours']['L'][1]['M']['close_time']['S'],
                                                              '%H:%M')
        form.deliveryCloseTimeTuesday.data = datetime.strptime(
            kitchen['delivery_hours']['L'][2]['M']['close_time']['S'], '%H:%M')
        form.deliveryCloseTimeWednesday.data = datetime.strptime(
            kitchen['delivery_hours']['L'][3]['M']['close_time']['S'], '%H:%M')
        form.deliveryCloseTimeThursday.data = datetime.strptime(
            kitchen['delivery_hours']['L'][4]['M']['close_time']['S'], '%H:%M')
        form.deliveryCloseTimeFriday.data = datetime.strptime(kitchen['delivery_hours']['L'][5]['M']['close_time']['S'],
                                                              '%H:%M')
        form.deliveryCloseTimeSaturday.data = datetime.strptime(
            kitchen['delivery_hours']['L'][6]['M']['close_time']['S'], '%H:%M')
        form.deliveryCloseTimeSunday.data = datetime.strptime(kitchen['delivery_hours']['L'][0]['M']['close_time']['S'],
                                                              '%H:%M')
    ...
    if dataFilter == 1:
        ...
        for order in sortedOrders:
            if order == sortedOrders[0]:
                ...
                for item in order['order_items']['L']:
                    if not any(
                            i['M']['meal_id']['S'] == item['M']['meal_id']['S'] for i in kitchen['order_items']['L']):
...
@app.route('/api/v1/meals/deliver/<string:order_id>', methods=['POST'])
@login_required
...
@app.route('/api/v1/meals/delete/<string:order_id>', methods=['POST'])
@login_required
